 ...
 *  - 객체지향적인 클래스로 설계하면 내부적으로 캡슐화 및 상속이 구현되어있음. 필요에 따라 equals,hashCode등의 함수를
 *    재정의 할 수도 있고, 클래스만의 고유한 메서드가 존재할 수도 있다.
 *    
 *  DTO(Data Transfer Object)
 *  - API서버간에 데이터 송수신하기 위한 용도의 객체를 정의하는 클래스
 *  - 어플리케이션의 비지니스로직에서 사용되는 VO 클래스들과 다르게 데이터 송수신 목적으로만 사용하기 때문에 
 *    불필요한 메서드가 존재하지 않는다. 오직 데이터 저장 및 조회기능의 메서드만 정의해둠.
 *  - API서버에서 제공하는 응답데이터를 DTO클래스로 정의하고 사용함
 *  - 거대 API서버 같은경우 (구글,카카오,네이버) 굉장히 많은 기능들(서비스 및 데이터)를 제공하고 있고 기능별로 필요로 하는
 *    요청주소 및 요청변수가 다르고 응답데이터 또한 다양하다. 이 응답데이터들을 모두 DTO클래스로 정희하게되면 너무 많은 클래스가 나옴
 *  - 너무 많은 클래스가 있다면 유지보수나 관리가 힘드므로 보통 정적 내부 클래스를 이용하여 하나의 클래스 파일로 묶어서 관리한다.
 *  
 * */
public class KakaoUserInforResponse {

	private Long id;
	private Boolean has_singed_up;
	...
	public Boolean getHas_singed_up() {
		return has_singed_up;
	}
	public void setHas_singed_up(Boolean has_singed_up) {
		this.has_singed_up = has_singed_up;
	}
