package com.kh.chap_02_map.part_01hashMap.run;
...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		/*
		 * HashMap ?
		 * 많은 양의 데이터를 빠르게 저장하고, 검색하기 위하여 key, value값을 함께 묶어서 관리하는
		 * Hash Table 알고리즘을 사용한 자료구조.
		 * 
		 * 계층구조를 보면 List나 Set계열은 Collection을 구현한 클래스. => 데이터 추가시에는 add메서드 사용.
		 ...
		 * 4. 동일한 key값으로 데이터를 저장하면 , 마지막으로 추가한 value로 치환된다.
...
//		System.out.println(s2.equals(s3));
		...
		System.out.println(hm); // 저장순서유지 x. value값은 중복되어도 상관 없음.
		
		hm.put("새우깡", new Snack("매운맛",700)); // key값이 중복된 경우 마지막의 value값으로 치환.
		System.out.println(hm);
		
		// 2. get(Object key) : V => Map에서 key값에 저장된 value값을 돌려주는 메소드.
		System.out.println(hm.get("새우깡"));
		
		// 3. size() => 컬렉션에 담겨있는 갯수
		System.out.println("size : "+hm.size()); // key값이 중복된경우 counting 안함.
		
		// 4. replace(K key, V value) => 컬렉션에 해당 key값을 찾아서 전달된 value로 변경시켜주는 메소드
		hm.replace("포테토칩", new Snack("겁나짠맛",1000));
		System.out.println(hm);
		
		// 5. remove(Object Key) => 컬렉션에 해당 key값을 찾아서 key값과 value값을 동시에 지워주는 메소드
		hm.remove("포테토칩");
		System.out.println(hm);
	    ...
	    // 1. keySet()
	    // keySet을 통해 HashMap에 저장된 key값들만 Set에 담아서 얻어오기.
	    // hm.keySet() : set
	    Set<String> keySet = hm.keySet();
	    
	    // 2. 1번 과정에서 작업된 set 을 Iterator에 담기
	    Iterator<String> itKey = keySet.iterator();
	    
	    // 3. Iterator반복문을 통해 내부값 추출
	    while(itKey.hasNext()) {
	    	String key = itKey.next(); // 새우깡 // 포테토칩 // 칸초
