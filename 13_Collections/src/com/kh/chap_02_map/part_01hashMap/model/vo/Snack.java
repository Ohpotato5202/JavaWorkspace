package com.kh.chap_02_map.part_01hashMap.model.vo;
...
public class Snack {
	 ...
	 * 메모리내에서 고유한 값을 가져가야 하는게 hashCode함수 hashCode로 반환된 값이 같다면 같은 객체.
	 * 
	 * ✔객체를 식별하는데 사용하는 고유한 값이 hashCode.✔✔
	 *  
	 * 이 함수를 오버라이딩 할때도 hashCode값이 같은 두 객체는 논리적으로 동일하다는 의미를 가지게 만들어야함.
	 * 두 객체간의 필드값이 동일하다면 같은 hashCode값이 반환되도록 오버라이딩 해야함.
	...
	/*
	 * equals() ?
	 * Object의 equals()메소드는 주소값과 주소값간의 동등비교.
	 * 
	 * 주소값 == 주소값이 아닌, 실제 참조하고 있는 주소값은 다르더라도,
	 * 두 객체간의 필드값이 동일한경우, 동일한 객체로 취급하고 싶다면 equals메소드를 오버라이딩 해줘야함
	 * 
	 * 즉, 객체간에 필드값이 동일하다면 true가 나올수 있도록 작성할 것.
	 * 
	 * a.equals(b)
	 * */
	...
	public boolean equals(Object obj) {
		...
		/*
		 * equals & haskCode는 동시에 오버라이딩 해야함.
		 * 
		 * equals함수는 객체와 객체간의 동등성을 비교하는 메소드
		 * 
		 * hashCode함수는 hash기반 자료구조인 hash table에서 객체를 "저장"하거나 "검색" 시 사용하는 함수.
		 * 
		 * hash테이블 ?
		 * 배열을 기반으로 하는 자료구조로 배열에 저장시킬 각 요소들에 대해 hashCode()함수를 호출하여 배열내부에
		 * 저장시킬 "인덱스"값을 구한다. => 일반적인 계산공식 [ hashCode()반환값 % 배열의 길이 ]
		 * 
		 * 따라서 두 객체간의 hashCode값이 같다면 같은 저장공간을 할당받음. 데이터 중복을 피할 수 있다.
		 *       두 객체간의 hashCode값이 같다면 같은 자료이므로, equals메서드도 true여야하고,
...
//		
//		// Snack s = (Snack) o;
//		// 만약 매개변수로 넘어온 객체가 Snack이 아닌경우 ClassCastException가 발생할수 있음.
