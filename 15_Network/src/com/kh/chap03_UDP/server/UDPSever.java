...
public class UDPSever {
	/*
	 * UDP?
	 *  - 비연결형 프로토콜로 수신자/발신자가 연결되어있든 아니든 상관 없이 데이터를 송/수신함.
	 *  - 데이터를 전달받은 쪽에서 제대로 수신이 완료되었는지 확인하지 않음. 즉, 데이터가 제대로 전달되지 않을 수 있다.
	 * 
	 *  - 번거로운 확인 절차를 모두 없앰으로써 데이터를 신속하게 송/수신이 가능하게 만들었음.
	 *    빠른 데이터 전송이 중요한 실시간 스트리밍이나, 유튜브 혹은 온라인게임에서 사용하는 방식.
	 * 
	 * User Datagram Protocol
	 *  - Datagram단위로 데이터를 전송하는 비연결지향형 프로토콜
	 *  - 클라이언트와 서버의 개념이 없다. 즉, 클라이언트의 요청을 받아주는 serversocker이 존재하지 않음.
	 ...
	 *  일반적으로 발신자/수신자 각각 하나의 DatagramSocket을 생성한후 이를 통해 데이터를 송/수신한다.
	 *  
	 *  DatagramPacket?
	 *  - UDP통신시 데이터를 포장할때 사용되는 클래스
	 *  - 내부에는 전송할 "데이터"와 발신자의 IP주소 및 포트번호가 포함되어있음.
	 *  - Datagram? 데이터를 전송하는 단위.
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket.receive(clientPacket); // 현재 소킷으로 데이터가 들어오기를 대기하는중
			
			System.out.println("클라이언트의 ip주소 ? "+clientPacket.getAddress().getHostAddress());
			System.out.println("클라이언트의 port번호 ? "+clientPacket.getPort());
			...
			socket.send(packet); // 상대방이 receive중이라면 내용 받기가 가능.
