package com.kh.chap02_TCP.sever;
 ...
 *   - 채팅, 이메일전송, 웹브라우징, 그 외에 안정적으로 데이터 송수신이 필요한 거의 모든 작업에 사용되는 프로토콜. 
 *   
 *   소켓?
 ...
 *   
 *   SeverSoket
 *   - 서버에서 생성하는 소켓으로 클라이언트의 연결요청이 들어오기 전까지 대기하다가, 요청이 들어오면 소켓 객체를 생성해준다.
 ...
 *   - IP주소와 포트번호를 통해 SeverSocket에 연결요청을 보냄.
...
public class TCPServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 클라이언트의 요청을 처리한 후 프로세스가 끊기지 않게 무한 반복
			int count = 0;
			while (true) {
				// 2) 3) 번 과정
				System.out.println("클라이언트 대기중");
				Socket client = server.accept(); // 클라이언트의 요청이 들어오기전까지 대기하다가 요청이 들어오면 소켓 객체를 생성후 반환
				...
				while(true) {
					...
					pw.println("클라이언트 측으로부터 메세지 전달을 받았습니다."); // 한줄 단위로 값을 출력
					// pw.flush();
				}
				pw.close();
				reader.close();
