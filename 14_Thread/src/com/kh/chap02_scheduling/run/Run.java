...
public class Run {
	 ...
	 *    - 코드로 우선순위 부여가 가능하다.
	 *    
	 * 2) 순환 할당 방식 스케줄링(OS의 기본 스케줄링 방식) --> Round Robin 방식
	 *    - 각 스레드에게 시간 할당량을 정하여 스레드를 정해진 시간만큼 실행시키는 방식
	 *    - 우선순위가 같은 스레드에는 순환 할당방식이 적용
	 *    - 코드로 제어가 불가능. => OS가 관리하므로.
	 *    
	 * 코드로 가능한것은 스레드 생성, 실행, 중지, 우선순위 설정정도만 가능하다. 
	 * */
	public static void main(String[] args) {
		
		Thread car = new Thread(new Car(), "Car");
		Thread plane = new Thread(new Plane(), "Palne");
        ...
        System.out.println(car.getName()+"우선순위 : "+car.getPriority());
        System.out.println(plane.getName()+"우선순위 : "+plane.getPriority());
        System.out.println(tank.getName()+"우선순위 : "+tank.getPriority());
         ...
         * 
         * main스레드가 종료시 다른 스레드도 종료시키게끔 하기 위해서는 setDaemon을 통해 주종관계 설정을 해줘야함.
         * 
         * 주의점은 스레드중에 하나라도 setDaemon으로 설정을 하지 않았다면 정상작동하지 않음.
         * 스레드를 start()시키기 전에 setDaemon설정을 해줘야함.(우선순위도 마찬가지)
         * */
